...
public class ItemFactory {
    ...
    public static Material fromLegacy(Material material) {
        if (isLegacy(material)) {
            return fromLegacy.apply(material);
        }
        return material;
    }
    ...
    public static boolean isSame(ItemStack source, ItemStack check) {
        ...
        if (use13) {
            if (source.getAmount() != check.getAmount()) {
                return false;
            }
            if (source.hasItemMeta() != check.hasItemMeta()) {
                return false;
            }
            if (fromLegacy(getType(source)) != fromLegacy(getType(check))) {
                return false;
            }
            ...
            if (!Objects.equals(i1.getDisplayName(), i2.getDisplayName())) {
                return false;
            }
            if (!Objects.equals(i1.getLore(), i2.getLore())) {
                return false;
            }
            if (source.getDurability() != check.getDurability()) {
                return false;
            }
            return Objects.equals(i1.getEnchants(), i2.getEnchants());
        } else {
            return source.equals(check);
        }
    }
