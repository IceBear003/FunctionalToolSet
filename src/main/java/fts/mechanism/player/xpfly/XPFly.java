package fts.mechanism.player.xpfly;
...
public class XPFly {
        ...
        task = new BukkitRunnable() {
            ...
            public void run() {
                ...
                for (UUID uuid : (ArrayList<UUID>) flyingPlayers.clone()) {
                    ...
                    if (player.hasPermission("fts.mechanism.player.xpfly.slowexhaust")) {
                        if (counter % 2 == 0) {
    ...
    public static void initXPFly(Player player) {
        if (!player.hasPermission("fts.mechanism.player.xpfly.use")) {
